...
class ModelManager:
    ...
    def __init__(self, args, data, pretrained_model=None):
        ...
        num_train_examples = len(data.train_all_examples)
    ...
    def get_score(self, y_true, y_pred, n_label_samples, args):
        if args.method == "classify":
            y_pred_ind = y_pred[0:n_label_samples]
            y_true_ind = y_true[0:n_label_samples]

            y_pred_ood = y_pred[n_label_samples:]
            y_true_ood = y_true[n_label_samples:]

            print(y_pred_ind)
            print(y_true_ind)

            print(y_pred_ood.size)
            print(y_true_ood.size)

            return {'ACC': round(clustering_accuracy_score(y_true, y_pred) * 100, 2),
                    'ACC_IND': round(accuracy_score(y_true_ind, y_pred_ind) * 100, 2),
                    'ACC_OOD': round(clustering_accuracy_score(y_true_ood, y_pred_ood) * 100, 2),
                    "F1": round(cluster_F1(y_true, y_pred) * 100, 2),
                    "F1_IND": round(cluster_F1(y_true_ind, y_pred_ind) * 100, 2),
                    "F1_OOD": round(cluster_F1(y_true_ood, y_pred_ood) * 100, 2)
                    }
            ...
            return {'ACC': round(accuracy_score(y_true, y_pred) * 100, 2),
                    'ACC_IND': round(accuracy_score(y_true_ind, y_pred_ind) * 100, 2),
                    'ACC_OOD': round(accuracy_score(y_true_ood, y_pred_ood) * 100, 2),
                    "F1": round(cluster_F1(y_true, y_pred) * 100, 2),
                    "F1_IND": round(cluster_F1(y_true_ind, y_pred_ind) * 100, 2),
                    "F1_OOD": round(cluster_F1(y_true_ood, y_pred_ood) * 100, 2)
                    }
    ...
    def tsne_visualization_2(self,x,y,predicted, args):
        ...
        TSNE_visualization(x, y, label_list, os.path.join(path, "pca_train_"+str(args.seed)+".pdf"))
    ...
    def save_results(self, args):
        ...
        var = [args.dataset, args.method, args.known_cls_ratio, args.IND_ratio, args.labeled_ratio, args.cluster_num_factor, args.seed, self.num_labels]
        names = ['dataset', 'method', 'known_cls_ratio', 'IND_ratio', 'labeled_ratio', 'cluster_num_factor','seed', 'K']
        vars_dict = {k:v for k,v in zip(names, var)}
        results = dict(self.test_results,**vars_dict)
        keys = list(results.keys())
        values = list(results.values())
        
        file_name = 'deepCluster.csv'
